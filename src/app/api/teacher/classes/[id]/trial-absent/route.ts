...
// POST /api/teacher/classes/[id]/trial-absent
// Body: { trialId: string }
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const result = await db.collection("trialBookings").updateOne(
      { _id: objectId },
      { $set: { status: "absent", updatedAt: new Date(), absentAt: new Date() } }
    );
    ...
    console.error("[teacher/classes/:id/trial-absent] POST error:", err);
